...
cleaned as (
    ...
    select
        ...
        cast(cardholder_present as bool) as cardholder_present,

        cast(mcc as string) as mcc,
        cast(is_cross_border as bool) as is_cross_border,
        cast(risk_score as float64) as risk_score,

        cast(processing_time_ms as int64) as processing_time_ms,
        cast(data_version as int64) as data_version,
        timestamp_micros(div(transaction_created_at, 1000))
        ...
        lower(trim(card_funding)) as card_funding,

        lower(trim(authorization_method)) as authorization_method,
        upper(trim(merchant_country)) as merchant_country,
        upper(trim(issuer_country)) as issuer_country,
        lower(trim(capture_method)) as capture_method,
        lower(trim(status)) as status,

        lower(trim(failure_code)) as failure_code,
        timestamp_micros(div(record_created_at, 1000)) as record_created_at,
        timestamp_micros(div(record_last_updated, 1000)) as record_last_updated,
